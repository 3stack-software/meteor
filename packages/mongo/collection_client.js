import { Random } from '../random/main_client.js';
import { Promise as Promise$1 } from '../promise/client.js';
import * as MongoID from '../mongo-id/id.js';
import * as EJSON from '../ejson/ejson.js';
import { check } from '../check/match.js';
import * as Match from '../check/match-namespace.js';
import { CollectionPrototype as AllowDenyCollectionPrototype } from '../allow-deny/allow-deny.js';
import { Meteor$_debug } from '../meteor/debug.js';
import { randomStream } from '../ddp-client/common/random_stream.js';
...
import * as Minimongo from '../minimongo/minimongo_common.js';
import { Meteor$connection } from '../ddp-client/client/client_convenience.js';
...
class Collection {
  constructor(name, options) {
    ...
    this._transform = Minimongo.wrapTransform(options.transform);
    ...
    if (options.defineMutationMethods !== false) {
      try {
        ...
        if (
          error.message ===
          `A method named '/${name}/insert' is already defined`
        )
  ...
  static __getCollectionByName(name) {
    if (typeof name !== 'string') {
      return;
    }
...
Object.assign(Collection.prototype, AllowDenyCollectionPrototype);
...
ASYNC_COLLECTION_METHODS.forEach(methodName => {
  ...
  Collection.prototype[methodNameAsync] = function (...args) {
    try {
      // TODO: Fibers remove this when we remove fibers.
      this[methodName].isCalledFromAsync = true;
      return Promise$1.resolve(this[methodName](...args));
    } catch (error) {
